...
class Sprites:
    def __init__(self):
        self.sprite_types = {
            'barrel': pygame.image.load('sprites/oil_barrel.png').convert_alpha(),
            'pedestal': pygame.image.load('sprites/BlackTorch.png').convert_alpha(),
            'devil': [pygame.image.load(f'sprites/demons/{i}.png').convert_alpha() for i in range(8)]
        }
        self.list_of_objects = [
            ...
            SpriteObject(self.sprite_types['devil'], False, (6, 3), -0.2, 0.7),
        ]


class SpriteObject:
    def __init__(self, p_object, static, pos, shift, scale):
